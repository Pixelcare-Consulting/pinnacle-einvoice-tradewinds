        ...
        while (!success && retryCount < lhdnConfig.maxRetries) {
          try {
            ...
            const mappedDocuments = result.map((doc) => ({
              ...
              // Add currency fields to fix foreign currency display issue
              currency: doc.currency || doc.documentCurrency,
              documentCurrency: doc.currency || doc.documentCurrency, // Map currency field to documentCurrency for frontend compatibility
            }));
...
const saveInboundStatus = async (data, req = null) => {
  ...
  const retryDatabaseOperation = async (
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        // Check if it's a deadlock or write conflict error
  ...
  for (const batch of batches) {
    // Process documents one by one to avoid deadlocks completely
    for (const item of batch) {
      try {
        ...
        await retryDatabaseOperation(async () => {
          return await prisma.wP_INBOUND_STATUS.upsert({
            where: { uuid: item.uuid },
            update: {
              submissionUid: item.submissionUid,
              longId: item.longId,
        ...
        // Synchronize status between inbound and outbound tables with retry logic
        if (item.status === "Failed") {
          await retryDatabaseOperation(async () => {
            return await prisma.wP_OUTBOUND_STATUS.updateMany({
              where: { UUID: item.uuid },
              data: {
                status: "Failed",
        ...
        successCount++;
        console.log(`Successfully processed document ${item.uuid}`);
      } catch (error) {
    ...
    try {
      ...
      const formattedDocuments = documents.map((doc) => {
        ...
        return {
          ...
          typeVersionName: doc.typeVersionName,
          documentStatusReason: doc.documentStatusReason,
          documentCurrency: doc.documentCurrency || doc.currency,
...
router.get("/documents/archive-staging", async (req, res) => {
  try {
    ...
    const mappedRecords = archiveRecords.map((record) => ({
      ...
      documentCurrency: record.currency || "MYR", // Map currency field to documentCurrency for frontend compatibility
      ...
      if (docsWithMissingData.length > 0) {
        ...
        for (const doc of docsWithMissingData) {
          try {
            ...
            if (supplierName) {
              // Update the database record using Prisma with retry logic
              await retryDatabaseOperation(async () => {
                return await prisma.wP_INBOUND_STATUS.update({
                  where: { uuid: doc.uuid },
                  data: {
