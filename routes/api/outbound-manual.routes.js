...
function normalizeIds(identifications) {
    const ids = { tin: '', brn: '', sst: '' };
    if (!identifications) return ids;
    ...
    if (typeof identifications === 'object') {
        return {
            tin: identifications.tin || '',
            brn: identifications.brn || '',
            sst: identifications.sst || '',
        };
    }
    return ids;
}
...
function compactParty(party) {
    if (!party) {
        return {
            company: '',
            identifications: normalizeIds(null),
            contact: {},
            address: {},
        };
    }
    ...
    return {
        company: party.name || party.company || party.registrationName || '',
        identifications: normalizeIds(party.identifications),
        contact: {
            email: party.contact?.email || party.email || null,
            phone: party.contact?.phone || party.phone || null,
        },
        address: {
            line: party.address?.line || party.address?.address || null,
...
function buildListInvoiceDetailsFromProcessedData(processedData) {
    return (processedData || []).map((doc) => ({
        ...
        supplier: compactParty(doc.supplier),
        buyer: compactParty(doc.buyer),
...
function formatExistingUploadBlock(existingFile, filename) {
    ...
    return `Row ${rowId}: A file named "${filename}" is still in your upload table (${label}). ${rowRef} first (My Uploads tab, search the filename). Changing the Excel name does not remove that row.`;
}
...
router.post('/upload-excel-template', [auth.isApiAuthenticated, excelUpload.single('excelFile')], async (req, res) => {
    try {
        ...
        if (duplicateCheck.summary.hasBlockingDuplicates) {
            ...
            const errorMessage =
                'Some invoice numbers in this file were already processed or submitted. Delete the blocking row in your upload table (My Uploads tab or search by filename) or use new invoice numbers. A new filename alone does not make them new invoices.';
    ...
    try {
        ...
        const transformedFiles = uploadedFiles.map((file, index) => {
            ...
            return {
                ...
                invoice_count: file.invoice_count || invoiceCount,
