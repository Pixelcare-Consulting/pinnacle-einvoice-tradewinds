...
function countUniqueParties(documents, field) {
    const seen = new Set();
    let count = 0;
    (documents || []).forEach((doc) => {
        const key = partyKey(doc[field]);
        if (key && !seen.has(key)) {
            seen.add(key);
            count += 1;
        }
...
class FileUploadManager {
    ...
    refreshTable(uploadedFileId) {
        ...
        manager.table.ajax.reload(() => {
            if (typeof manager.applyTeamViewFilter === 'function') {
                manager.applyTeamViewFilter();
            }
            if (uploadedFileId) {
                manager.table.order([[7, 'desc'], [11, 'desc']]).page(0).draw('page');
                setTimeout(() => highlightRowByFileId(uploadedFileId), 150);
            }
        }, false);
    }
...
class InvoiceTableManager {
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        render: (data, type) => {
                            ...
                            return this.renderUploadedDate(data);
                        }
                    },
                    {
                        data: 'invDateInfo',
    ...
    initializeTeamViewFilter() {
        document.querySelectorAll('.team-view-filters [data-team-view]').forEach((button) => {
            button.addEventListener('click', (e) => {
                document.querySelectorAll('.team-view-filters [data-team-view]').forEach((btn) =>
    ...
    generateFileInfoContent(fileData) {
        ...
        if (fileData.invoiceNumber || resolveInvoiceCount(fileData) > 0) {
            const invoiceCount = resolveInvoiceCount(fileData);
            const details = fileData.invoiceDetails || fileData.metadata?.listInvoiceDetails || [];
            const receiverCount = countUniqueParties(details, 'buyer');
            const supplierCount = countUniqueParties(details, 'supplier');
            content += `<div class="detail-item">
                <strong>Invoice Count:</strong> ${invoiceCount}
            </div>`;
            if (invoiceCount > 0 && (receiverCount > 0 || supplierCount > 0)) {
                const parts = [`${invoiceCount} invoice${invoiceCount === 1 ? '' : 's'}`];
                if (receiverCount > 0) {
                    parts.push(`${receiverCount} unique receiver${receiverCount === 1 ? '' : 's'}`);
                }
                if (supplierCount > 0) {
                    parts.push(`${supplierCount} supplier${supplierCount === 1 ? '' : 's'}`);
                }
                content += `<div class="detail-item text-muted">
    ...
    // Show receiver modal
    async showReceiverModal(uniqueId) {
    ...
    extractSupplierData(documents) {
        ...
        documents.forEach(doc => {
            if (doc.supplier) {
                const supplierKey = partyKey(doc.supplier);
                if (supplierKey && !seenSuppliers.has(supplierKey)) {
            ...
            if (result?.success) {
                ...
                await showSubmissionSuccessModal({
                    ...
                    guidance: [
                        ...
                        'Your .xlsx file is kept on disk — search by filename in the table to review it.'
                    ]
                });
                notifyInboundModule({
                ...
                if (window.toastNotification?.info) {
                    window.toastNotification.info(
                        'File kept',
                        'Search by filename in the upload table to find your file.',
                        5000
                    );
                }
        ...
        try {
            ...
            await showSubmissionSuccessModal({
                ...
                guidance: [
                    ...
                    'Your .xlsx files are kept on disk — search by filename in the table to review them.'
                ]
            });
            ...
            if (window.toastNotification?.info) {
                window.toastNotification.info(
                    'Files kept',
                    'Search by filename in the upload table to find submitted uploads.',
                    5000
                );
            }
