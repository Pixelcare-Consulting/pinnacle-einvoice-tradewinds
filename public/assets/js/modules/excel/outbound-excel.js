...
class InvoiceTableManager {
    ...
    // }
     ...
     renderCompanyInfo(data, showCompanyInfo = true) {
        if (!showCompanyInfo || !data) return '';
        return `
            <div class="cell-group">
                <div class="cell-main">
                    <i class="bi bi-building me-1"></i>
    ...
    async exportSelectedRecords() {
        try {
            ...
            const exportData = selectedRows.map(row => {
                // Extract currency and amount from totalAmount field
                let totalAmountDisplay = row.totalAmount;
                if (row.totalAmount && typeof row.totalAmount === 'string') {
                    // If totalAmount already contains currency (e.g., "USD 123.45"), use it as is
                    totalAmountDisplay = row.totalAmount;
                } else if (row.totalAmount) {
                    // If it's just a number, format it with currency from row data or MYR as fallback
                    const currency = row.currency || 'MYR';
                    totalAmountDisplay = `${currency} ${parseFloat(row.totalAmount).toFixed(2)}`;
                }
...
function generateLineItemsRows(lhdnJson) {
    try {
        const invoiceLines = lhdnJson?.Invoice?.[0]?.InvoiceLine || [];
        const currency = lhdnJson?.Invoice?.[0]?.DocumentCurrencyCode?.[0]?._ || '';
...
async function performStep2(data, version) {
    try {
        ...
        try {
            ...
            // Notify inbound module about new submission
            notifyInboundModule({
                fileName,
                type,
                company,
            ...
            // Start real-time status monitoring for this submission
            if (result.submissionUid || result.uuid) {
                startRealTimeStatusMonitoring(fileName, result.submissionUid || result.uuid);
            }

            // Inbound module will automatically refresh via LHDN polling
...
function notifyInboundModule(submissionData) {
    try {
        ...
        console.log('Notified inbound module about new submission:', submissionData);
    } catch (error) {
...
/**
 * Start real-time status monitoring for a submission
 * @param {string} fileName - The file name
 * @param {string} submissionUid - The submission UID
 */
function startRealTimeStatusMonitoring(fileName, submissionUid) {
    console.log(`🔄 Starting real-time status monitoring for ${fileName} (${submissionUid})`);

    const monitoringKey = `status_monitor_${submissionUid}`;
    ...
    localStorage.setItem(monitoringKey, JSON.stringify(monitoringInfo));

    // Start monitoring
    monitorSubmissionStatus(submissionUid, monitoringInfo);
}

/**
 * Monitor submission status with polling
 * @param {string} submissionUid - The submission UID
 * @param {Object} monitoringInfo - Monitoring configuration
 */
async function monitorSubmissionStatus(submissionUid, monitoringInfo) {
    const monitoringKey = `status_monitor_${submissionUid}`;

    try {
        monitoringInfo.attempts++;
        monitoringInfo.lastCheck = Date.now();

        console.log(`🔍 [Monitor ${monitoringInfo.attempts}/${monitoringInfo.maxAttempts}] Checking status for ${submissionUid}`);

        // Update monitoring info
        localStorage.setItem(monitoringKey, JSON.stringify(monitoringInfo));

        // Check status via API
        const response = await fetch(`/api/outbound-files/polling-status?submissionUid=${submissionUid}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
            }
        ...
        if (response.ok) {
            ...
            if (data.success && data.statuses && data.statuses.length > 0) {
                ...
                console.log(`📊 [Monitor] Status for ${submissionUid}:`, latestStatus.status);

                // Check if polling is complete
                if (['completed', 'failed', 'max_attempts_reached'].includes(latestStatus.status)) {
                    console.log(`✅ [Monitor] Monitoring complete for ${submissionUid}: ${latestStatus.status}`);

                    // Update monitoring info
                    monitoringInfo.status = 'completed';
                    monitoringInfo.finalStatus = latestStatus.status;
                    monitoringInfo.completedAt = Date.now();
                    localStorage.setItem(monitoringKey, JSON.stringify(monitoringInfo));

                    // Notify inbound module about status update
                    notifyStatusUpdate(submissionUid, latestStatus);
        ...
        if (monitoringInfo.attempts < monitoringInfo.maxAttempts) {
            console.log(`⏰ [Monitor] Scheduling next check for ${submissionUid} in ${monitoringInfo.interval}ms`);

            setTimeout(() => {
                monitorSubmissionStatus(submissionUid, monitoringInfo);
            }, monitoringInfo.interval);
        } else {
            console.log(`⚠️ [Monitor] Max attempts reached for ${submissionUid}`);

            monitoringInfo.status = 'max_attempts_reached';
            monitoringInfo.completedAt = Date.now();
            localStorage.setItem(monitoringKey, JSON.stringify(monitoringInfo));
        }

    } catch (error) {
        console.error(`❌ [Monitor] Error checking status for ${submissionUid}:`, error);

        // Retry on error if under max attempts
        if (monitoringInfo.attempts < monitoringInfo.maxAttempts) {
            setTimeout(() => {
                monitorSubmissionStatus(submissionUid, monitoringInfo);
            }, monitoringInfo.interval * 2); // Double interval on error
        }
    }
}
 ...
 */
function notifyStatusUpdate(submissionUid, statusInfo) {
    try {
        const notification = {
            type: 'status_update',
            timestamp: Date.now(),
        ...
        localStorage.setItem('status_update_notification', JSON.stringify(notification));
        ...
        console.log('📢 Notified about status update:', notification);
    } catch (error) {
        console.error('❌ Error notifying about status update:', error);
    }
}

// Outbound refresh trigger removed - inbound module now uses live LHDN data only
